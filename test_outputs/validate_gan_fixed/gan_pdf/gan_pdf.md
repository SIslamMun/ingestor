...
$$\begin{aligned}
C (G) & = \max_{D} V (G, D) \\ & = \mathbb{E}_{x \sim p_{\text{data}}} [ \log D_{G}^{*} (x) ] + \mathbb{E}_{x \sim p_{G}} [ \log(1 - D_{G}^{*} (G (z))) ] \\ & = \mathbb{E}_{x \sim p_{\text{data}}} [ \log D_{G}^{*} (x) ] + \mathbb{E}_{x \sim p_{G}} [ \log(1 - D_{G}^{*} (x)) ] \\ & = \mathbb{E}_{x \sim p_{\text{data}}} \left [ \log \frac{ p_{\text{data}}} { p_{\text{data}} (x) + p_{g} (x)} \right ] + \mathbb{E}_{x \sim p_{g}} \left [ \log \frac{ p_{g} (x)} { p_{\text{data}} (x) + p_{g} (x)} \right ]
\end{aligned}$$
...
$$\mathbb{E}_{x \sim p_{\text{data}}} [ - \log 2 ] + \mathbb{E}_{x \sim p_{g}} \left [ - \log 2 \right ] = - \log 4$$

and that by subtracting this expression from C ( G ) = V ( D ∗ G , G ) , we obtain:

$$C (G) = - \log(4) + KL \left (p_{\text{data}} \left \| \frac{ p_{\text{data}} + p_{G}} { 2} \right) + KL \left (p_{g} \left \| \frac{ p_{G} + p_{g}} { 2} \right) \right)$$
