...
class PMCClient(BaseClient):
    ...
    async def doi_to_pmcid(self, doi: str) -> Optional[str]:
        ...
        headers = {
            "User-Agent": "ingestor/1.0 (https://github.com/shazzadhk/ingestor; mailto:contact@example.com)",
        }

        try:
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                response = await client.get(self.IDCONV_URL, params=params, headers=headers)
    ...
    async def get_metadata(self, pmcid: str) -> Optional[dict[str, Any]]:
