...
$$\min_{G \D} \max_{D} V (D ; G) = \mathbb{E}_{x \sim p_{D} (x)} [ \log D (x) ] + \mathbb{E}_{z \sim p_{z} (z)} [ \log(1 - D (G (z))) ] .$$
...
$$\nabla_{\theta_{d}} \frac{ 1} { m} \sum_{i = 1}^{m} \left [ \log D \left (x^{(i)} \right) + \log \left (1 - D \left (G \left (z^{(i)} \right) \right) \right) \right) \right ] .$$
...
$$\nabla_{\theta_{s}} \frac{ 1} { m} \sum_{i = 1}^{m} \log \left (1 - D \left (G \left (z^{(i)} \right) \right) \right) .$$
...
$$V (G, D) & = \int_{x} p_{\text{data}} (x) \log(D (x)) d x + \int_{z} p_{z} (z) \log(1 - D (g (z))) d z \\ & = \int_{x} p_{\text{data}} (x) \log(D (x)) + p_{g} (x) \log(1 - D (x)) d x$$
...
$$C (G) & = \max_{D} V (G, D) \\ & = \mathbb{E}_{x \sim p_{\} p a} [ \log D_{G}^{*} (x) ] + \mathbb{E}_{x \sim p_{G}} [ \log(1 - D_{G}^{*} (G (z))) ] \\ & = \mathbb{E}_{x \sim p_{\} p a} [ \log D_{G}^{*} (x) ] + \mathbb{E}_{x \sim p_{G}} [ \log(1 - D_{G}^{*} (x)) ] \\ & = \mathbb{E}_{x \sim p_{\} p a} \left [ \log \frac{ p_{\} p a} { P_{\} p a (x) + p_{g} (x)} \right ] + \mathbb{E}_{x \sim p_{g}} \left [ \log \frac{ p_{g} (x)} { p_{\} p a (x) + p_{g} (x)} \right ]$$
...
$$\mathbb{E}_{x \sim p_{d a t i}} [ - \log 2 ] + \mathbb{E}_{x \sim p_{g}} \left [ - \log 2 \right ] = - \log 4$$

and that by subtracting this expression from C ( G ) = V ( D ∗ G , G ) , we obtain:

$$C (G) = - \log(4) + K L \left (p_{data} \left \| \frac{ p_{data} + p_{_{G}}} { 2} \right) + K L \left (p_{g} \left \| \frac{ p_{_{G}} + p_{_{g}}} { 2} \right) \right)$$
...
$$C (G) = - \log(4) + 2 \cdot J S D \left (p_{\text{data}} \left \| p_{g} \right)$$
...
$$\mathbb{E}_{x \sim p_{d o t a}} [ \log D_{G}^{*} (x) ] + \mathbb{E}_{x \sim p_{g}} [ \log(1 - D_{G}^{*} (x)) ]$$
...
We trained adversarial nets an a range of datasets including MNIST[[23]](#ref-23), the Toronto Face Database (TFD) [[28]](#ref-28), and CIFAR-10 [[21]](#ref-21). The generator nets used a mixture of rectifier linear activations [19, 9] and sigmoid activations, while the discriminator net used maxout [[10]](#ref-10) activations. Dropout [[17]](#ref-17) was applied in training the discriminator net. While our theoretical framework permits the use of dropout and other noise at intermediate layers of the generator, we used noise as the input to only the bottommost layer of the generator network.
...
| Model            | MNIST       | TFD       |
|------------------|-------------|-----------|
| DBN [[3]](#ref-3)          | 138 ± 2     | 1909 ± 66 |
| Stacked CAE [[3]](#ref-3)  | 121 ± 1 . 6 | 2110 ± 50 |
| Deep GSN [[6]](#ref-6)     | 214 ± 1 . 1 | 1890 ± 29 |
| Adversarial nets | 225 ± 2     | 2057 ± 26 |

of the Gaussians was obtained by cross validation on the validation set. This procedure was introduced in Breuleux et al. [[8]](#ref-8) and used for various generative models for which the exact likelihood is not tractable [25, 3, 5]. Results are reported in Table 1. This method of estimating the likelihood has somewhat high variance and does not perform well in high dimensional spaces but it is the best method available to our knowledge. Advances in generative models that can sample but not estimate likelihood directly motivate further research into how to evaluate such models.
...
2. Learned approximate inference can be performed by training an auxiliary network to predict z given x . This is similar to the inference net trained by the wake-sleep algorithm [[15]](#ref-15) but with the advantage that the inference net may be trained for a fixed generator net after the generator net has finished training.

glyph[negationslash]

3. One can approximately model all conditionals p ( x S | x S ) where S is a subset of the indices of x by training a family of conditional models that share parameters. Essentially, one can use adversarial nets to implement a stochastic extension of the deterministic MP-DBM [[11]](#ref-11).
